...
var go_button_g2 = grd3.append("svg").attr("height",10*square10);
...
grd3.selectAll("line").remove();
  ...
  grd3.selectAll("rect").each(function(){
    ...
    if (nm[2] == 1){
      grd3.append("line")
      .attr("x1", parseFloat(cr.attr("x")-1))
      .attr("x2", square10sm+(parseFloat(cr.attr("x"))+1 + (nm[3] == 7 ? 0 : 1)))
      .attr("y1", parseFloat(cr.attr("y")))
      .attr("y2",parseFloat(cr.attr("y")))
      .style("stroke-width", 2)
      .attr("stroke","#333");
    ...
    if (nm[3] == 1){
      ...
      .attr("x2", (parseFloat(cr.attr("x"))))
      .attr("y1", parseFloat(cr.attr("y")-1))
      .attr("y2",square10sm+(parseFloat(cr.attr("y"))+1 + (nm[2] == 7 ? 0 : 1)))
      .style("stroke-width", 2)
      .attr("stroke","#333");
    }
    else if (nm[3] == 7){
...
var helptxt = grd4.append("svg").attr("height",11*square10).attr("width",300).attr("x",0);
